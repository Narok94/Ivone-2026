...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const c = req.body;
      const result = await pool.query(
        'UPDATE clientes SET full_name=$1, phone=$2, email=$3, street=$4, neighborhood=$5, city=$6, state=$7, cep=$8, number=$9, complement=$10, cpf=$11, observation=$12 WHERE id=$13 RETURNING *',
        [c.fullName, c.phone, c.email, c.street, c.neighborhood, c.city, c.state, c.cep, c.number, c.complement, c.cpf, c.observation, id]
      );
